...
// const selected = [];
//   document.querySelectorAll('.token.active').forEach(token => {
//     selected.push(toppobj[token.textContent]);
//   });



const form = document.getElementById('eventForm');
const form2 = document.getElementById('paymentForm');
...
form.addEventListener('submit', async (e) => {
  ...
  document.querySelectorAll('.token.active').forEach(token => {
    selectedEvents.push(toppobj[token.textContent]);
  });
  ...
  // POST request
  try {
    const response = await fetch('https://gjbackend-zuf6.onrender.com/api/register', {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      sessionStorage.setItem('formData', JSON.stringify(responseData));
      if (responseData["success"]) {
        document.getElementById('regis').style.display = 'none';
        document.getElementById('idtag').innerHTML = responseData["id"];
        document.getElementById('totaltag').innerHTML = responseData["amount"];
        document.getElementById('payval').style.display = 'block';
        scrollToSection('payval');
      }
    } else {
      location.reload();
...
form2.addEventListener('submit', async (e) => {
  ...
  const paydata = JSON.parse(sessionStorage['formData']);
  console.log(paydata);
  const payload2 = {
    participant_id: paydata["id"],
    tid: formData2.get('tid'),
    encrypted: paydata["encrypted"],
    ipu: paydata["ipu"]
  };
  ...
  // POST request
  try {
    const response2 = await fetch('https://gjbackend-zuf6.onrender.com/api/txnid', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (response2.ok) {
      ...
      console.log("running")
      console.log('Server Response:', responseData2);
      if (responseData2["success"]) {
        document.getElementById('payval').style.display = 'none';
